...
const cmdHTTPLong = `Start the Sovereign HTTP service only, providing RESTful API interfaces for message delivery and management.
...
After starting the service, Sovereign HTTP will listen on the configured HTTP port (default: 0.0.0.0:10080,
configurable via --http-address) and provide RESTful API interfaces for client access.`

func NewCmd() *cobra.Command {
	runCmd := &cobra.Command{
		Use:   "http",
		Short: "Run the Sovereign HTTP service only",
		Long:  cmdHTTPLong,
